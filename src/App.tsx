...
const Glass = ({ children, className = "" }: { children: ReactNode; className?: string }) => (
  <div className={`backdrop-blur-xl bg-white/80 border border-white shadow-2xl shadow-slate-200/50 ${className}`}>
    {children}
  </div>
);
...
const Pill = ({ active, children, onClick, }: { active?: boolean; children: ReactNode; onClick?: () => void; }) => (
  <button
    ...
    className={`px-4 py-2 rounded-full text-xs font-black uppercase tracking-widest transition-all ${
      active ? "bg-slate-900 text-white shadow-lg scale-105" : "bg-white text-slate-400 hover:text-slate-600 border border-slate-100"
    }`}
  >
    {children}
  </button>
...
const Toggle = ({
  ...
  <div className="flex items-center justify-between p-6 rounded-[32px] bg-white border border-slate-50 shadow-sm">
    <div className="flex items-center gap-4">
      ...
      <div>
        ...
        {hint && <div className="text-[10px] font-bold text-slate-400 uppercase tracking-tight">{hint}</div>}
      </div>
    </div>
    <button
...
const FamilyStrip = () => (
  <div className="flex -space-x-3">
    {FAMILY_MEMBERS.map((m) => (
      <div key={m.name} className="relative group cursor-help">
        <div className={`w-10 h-10 rounded-2xl border-2 border-white overflow-hidden shadow-sm transition-transform group-hover:-translate-y-1 ${m.color}`}>
          <img
            src={m.src}
            alt={m.name}
...
const CinemaHero = ({
  ...
  return (
    <div className="relative h-[80vh] w-full overflow-hidden rounded-b-[64px] shadow-2xl bg-slate-900 group">
      <img
        src={src}
        alt="Hero"
        className="absolute inset-0 w-full h-full object-cover scale-105 group-hover:scale-100 transition-transform duration-1000 ease-out opacity-70"
      />
      ...
      <div className="absolute top-0 left-0 right-0 p-8 flex justify-between items-start z-20">
        <div className="flex flex-col gap-1">
          <div className="text-[10px] font-black text-white/50 uppercase tracking-[0.3em] mb-1">
            24 Juil → 18 Août
          </div>
          <div className="text-2xl font-black text-white tracking-tighter flex items-center gap-2">
            Vietnam <span className="text-white/30 italic font-light">2026</span>
          </div>
        </div>
        <button
          onClick={onOpenQuick}
          className="w-12 h-12 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center text-white active:scale-95 transition-all shadow-xl"
        >
          <Search size={20} />
        </button>
      </div>
      ...
      {/* Center Content */}
      <div className="absolute inset-x-0 bottom-24 px-8 z-10">
        <div className="flex flex-col gap-6">
          <div className="flex flex-wrap gap-2">
            {TRIP_DATA.meta.vibe.map((v) => (
              <span key={v} className="px-3 py-1 rounded-full bg-white/10 backdrop-blur-sm text-[10px] font-black text-white uppercase tracking-widest border border-white/10">
                {v}
              </span>
            ))}
          </div>
          <div>
            <div className="text-[10px] font-black text-white/40 uppercase tracking-[0.4em] mb-3">Focus du moment :</div>
...
const QuickSheet = ({
  ...
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[100] p-6 animate-in fade-in zoom-in-95 duration-200">
      ...
      <div className="relative h-full flex flex-col justify-center">
        <div className="grid grid-cols-2 gap-4">
          ...
          >
            <Lightbulb size={32} />
            <div>
              <div className="text-lg font-black leading-tight">Bon à savoir</div>
              <div className="text-xs font-bold text-white/60 uppercase">Checklist + Argent</div>
            </div>
          </button>
          <button
          ...
          >
            <Utensils size={32} />
            <div>
              <div className="text-lg font-black leading-tight">Food + Logistique</div>
              <div className="text-xs font-bold text-white/60 uppercase">Aéroports + Plats</div>
            </div>
          </button>
          <a
          ...
          >
            <Smartphone size={32} />
            <div>
              <div className="text-lg font-black leading-tight">Grab (rappel)</div>
              <div className="text-xs font-bold text-slate-400 uppercase">Taxi & Food</div>
            </div>
          </a>
          <button
          ...
          >
            <Wallet size={32} />
            <div>
              <div className="text-lg font-black leading-tight">Budget</div>
              <div className="text-xs font-bold text-white/60 uppercase">Totaux + Tickets</div>
            </div>
          </button>
        </div>
        <button
          onClick={onClose}
          className="mt-12 w-16 h-16 rounded-full bg-white/10 text-white flex items-center justify-center self-center"
        >
          <X size={32} />
        </button>
      </div>
...
const CityTimeline = ({
  ...
  return (
    <div className="flex gap-3 overflow-x-auto pb-6 px-8 no-scrollbar scroll-smooth">
      {cities.map((c) => (
        <button
          key={c}
          onClick={() => onSelect(c)}
...
const DayCardMobile = ({
  ...
  return (
    <div className="group relative bg-white rounded-[40px] overflow-hidden shadow-sm border border-slate-50 transition-all hover:shadow-xl hover:shadow-slate-200/50 p-3 mb-6">
      <div className="relative h-48 w-full rounded-[32px] overflow-hidden mb-6 bg-slate-100">
        <img
          src={coverSrc}
          alt={day.city}
        ...
        <button
          ...
          className={`absolute top-4 right-4 w-10 h-10 rounded-full flex items-center justify-center transition-all ${
            isFav ? "bg-pink-500 text-white" : "bg-white/20 backdrop-blur-md text-white border border-white/20"
          }`}
        >
      ...
      <div className="px-5 pb-5">
        ...
        <h4 className="text-2xl font-black text-slate-900 tracking-tighter mb-4 leading-none">
          {day.city}
        </h4>
        
        <div className="flex flex-wrap gap-1.5 mb-6">
          {day.theme.map((t) => (
            <span key={t} className="px-2.5 py-1 rounded-full bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-tight">
              {t}
            </span>
          ))}
        </div>
        ...
        <div className="space-y-4">
          {day.blocks.map((b, idx) => {
            if (isFatigue && b.label === "Soir" && !b.plan.toLowerCase().includes("repos")) {
              return (
                <div key={idx} className="flex gap-4 opacity-40">
                  <div className="w-px h-12 bg-slate-100 mt-2" />
                  <div>
                    <div className="text-[10px] font-black text-slate-300 uppercase">{b.label}</div>
                    <div className="text-xs text-slate-300 line-through italic">Repos suggéré ce soir 😴</div>
                  </div>
                </div>
              );
            }
            ...
            if (kidsMode && shouldHideImpact(b.plan)) {
              return (
                <div key={idx} className="bg-amber-50/50 p-4 rounded-3xl border border-amber-100/50">
                  <div className="text-[10px] font-black text-amber-600 uppercase mb-1">{b.label}</div>
                  <div className="text-xs text-amber-700 font-bold italic">Contenu masqué (Mode Kids)</div>
                </div>
              );
            }
            ...
            return (
              <div key={idx} className="flex gap-4">
                <div className="flex flex-col items-center">
                  ...
                  {idx < day.blocks.length - 1 && <div className="w-px flex-1 bg-slate-100 my-1" />}
                </div>
                <div>
                  <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{b.label}</div>
                  <p className="text-sm font-bold text-slate-700 leading-snug">{b.plan}</p>
                  {b.links?.length ? (
        ...
        {mood === "energy" && (
          <div className="mt-8 p-4 rounded-3xl bg-amber-50 border border-amber-100 text-center">
            <div className="text-[10px] font-black text-amber-600 uppercase tracking-widest mb-1">Boost Énergie</div>
            <div className="text-xs font-bold text-amber-700 italic">Énergie au max : on explore un café caché !</div>
          </div>
        )}
      </div>
    </div>
...
const HotelCard = ({ hotel }: { hotel: HotelItem }) => {
  ...
  return (
    <div className="bg-white rounded-[40px] p-4 shadow-sm border border-slate-50 transition-all hover:shadow-xl hover:shadow-slate-200/50 mb-6">
      <div className="relative h-64 w-full rounded-[32px] overflow-hidden mb-6 bg-slate-100">
        {hotel.cover ? (
          <img
            src={hotel.cover}
            alt={hotel.name}
          ...
          <div className="w-full h-full flex items-center justify-center text-slate-300 uppercase text-xs font-black italic">Photo</div>
        )}
        <div className="absolute top-4 left-4 px-4 py-2 rounded-2xl bg-white/20 backdrop-blur-md border border-white/20 text-white text-[10px] font-black uppercase tracking-widest">
          {hotel.city}
        </div>
      </div>

      <div className="px-4 pb-4">
        <h4 className="text-2xl font-black text-slate-900 tracking-tighter leading-tight mb-2">
          {hotel.name}
        </h4>
        <div className="text-xs font-bold text-indigo-500 mb-6">{hotel.dates}</div>

        {hotel.note && (
          <div className="mb-6 p-4 rounded-3xl bg-slate-50 text-[11px] font-bold text-slate-500 italic">
            {hotel.note}
          </div>
        )}

        <p className="text-sm font-bold text-slate-600 italic mb-8 leading-relaxed">
          “{hotel.why}”
        </p>

        <div className="grid grid-cols-2 gap-3 mb-8">
          <div className="p-5 rounded-3xl bg-slate-50 border border-slate-100">
            <div className="text-[10px] font-black text-slate-400 uppercase tracking-tight mb-1">Famille</div>
            <div className="text-lg font-black text-slate-900">{formatUSD(hotel.budget.us)}</div>
          </div>
          <div className="p-5 rounded-3xl bg-indigo-50 border border-indigo-100">
            <div className="text-[10px] font-black text-indigo-400 uppercase tracking-tight mb-1">Claudine</div>
            <div className="text-lg font-black text-indigo-900">{formatUSD(hotel.budget.claudine)}</div>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          {link ? (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex-1 py-5 rounded-[28px] bg-slate-900 text-white text-xs font-black uppercase tracking-widest text-center shadow-lg active:scale-95 transition-all"
            >
              Voir Résa
            </a>
          ) : (
            <div className="flex-1 py-5 rounded-[28px] bg-slate-100 text-slate-400 text-[10px] font-black uppercase tracking-widest text-center">Pas de lien</div>
          )}
          <a
            href={googleMapsSearchUrl(hotel.name)}
            target="_blank"
            rel="noopener noreferrer"
            className="w-16 h-16 rounded-[28px] bg-slate-100 flex items-center justify-center text-slate-900 shadow-sm active:scale-95 transition-all"
          >
            <MapPin size={24} />
          </a>
        </div>
...
const TipsChecklist = () => {
  ...
  return (
    <div className="bg-white rounded-[40px] p-8 shadow-sm border border-slate-50 mb-8 overflow-hidden">
      <div className="flex justify-between items-end mb-8">
        <div>
          <h4 className="text-2xl font-black tracking-tighter text-slate-900 leading-none mb-1">Essentiels</h4>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Avant de décoller</p>
        </div>
        <div className="text-4xl font-black text-indigo-100 tracking-tighter">{progress}% prêt</div>
      </div>
        ...
        {ESSENTIALS_CHECKLIST.map((item) => (
          ...
          >
            <div className={`w-6 h-6 rounded-xl border-2 flex items-center justify-center transition-all ${
              checked.includes(item) ? "bg-indigo-500 border-indigo-500 text-white" : "bg-white border-slate-200"
            }`}>
              {checked.includes(item) && <CheckSquare size={14} />}
            </div>
            <span className={`text-sm font-black text-left ${checked.includes(item) ? "text-slate-400 line-through" : "text-slate-700"}`}>
              {item}
            </span>
          </button>
        ))}
...
const SimpleListCard = ({ title, icon, items }: { title: string; icon: ReactNode; items: string[] }) => (
  <div className="bg-white rounded-[40px] p-8 shadow-sm border border-slate-50 mb-8">
    ...
    <h4 className="text-2xl font-black tracking-tighter text-slate-900 mb-6">{title}</h4>
    <div className="space-y-4">
      {items.map((t, i) => (
        <div key={i} className="flex gap-4">
          <div className="w-1.5 h-1.5 rounded-full bg-slate-200 mt-2 flex-shrink-0" />
          <p className="text-sm font-bold text-slate-600 leading-snug">{t}</p>
        </div>
      ))}
    </div>
  </div>
...
const PhrasebookCard = () => (
  <div className="bg-slate-900 rounded-[40px] p-8 text-white mb-8 shadow-2xl shadow-slate-900/30">
    <div className="flex justify-between items-start mb-10">
      <div>
        <h4 className="text-2xl font-black tracking-tighter leading-none mb-1">Mots utiles</h4>
        <p className="text-[10px] font-bold text-white/30 uppercase tracking-widest italic">Apprendre & Rire</p>
      </div>
    ...
    <div className="space-y-6">
      {PHRASEBOOK.map((p) => (
        <div key={p.fr} className="flex justify-between items-center group">
          <div>
            <div className="text-[10px] font-black text-white/40 uppercase tracking-tight mb-0.5">{p.fr}</div>
            <div className="text-lg font-black tracking-tight">{p.vi}</div>
          </div>
          <div className="px-4 py-2 rounded-2xl bg-white/5 border border-white/10 text-xs font-bold text-indigo-400">
            • {p.phon}
          </div>
        </div>
      ))}
...
const AirportGlossary = () => (
  <div className="bg-white rounded-[40px] p-8 shadow-sm border border-slate-50 mb-8">
    <h4 className="text-2xl font-black tracking-tighter text-slate-900 mb-2">Aéroports</h4>
    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-10">Codes + trajets estimés</p>
    <div className="space-y-10">
      {AIRPORT_GLOSSARY.map((a, i) => (
        <div key={i} className="relative">
          <div className="flex justify-between items-start mb-4">
            <div className="flex items-center gap-3">
              <div className="px-3 py-1 rounded-xl bg-indigo-50 text-indigo-600 text-xs font-black">{a.code}</div>
              <div className="text-sm font-black text-slate-900"> • {a.city}</div>
            </div>
          </div>
          <div className="text-xs font-bold text-slate-400 mb-6 italic leading-relaxed">{a.airport}</div>
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 rounded-3xl bg-slate-50 border border-slate-100">
              <div className="text-[9px] font-black text-slate-400 uppercase tracking-tight mb-1">Depuis hôtel</div>
              <div className="text-xs font-black text-slate-900">{a.fromHotel}</div>
            </div>
            <div className="p-4 rounded-3xl bg-slate-50 border border-slate-100">
              <div className="text-[9px] font-black text-slate-400 uppercase tracking-tight mb-1">Trajet</div>
              <div className="text-xs font-black text-slate-900">{a.eta}</div>
            </div>
          </div>
          {a.note && (
            <div className="mt-4 text-[10px] font-black text-indigo-500 uppercase tracking-tight">
              ! {a.note}
            </div>
          )}
        </div>
    ...
    <div className="min-h-screen bg-slate-50 font-sans selection:bg-indigo-100 selection:text-indigo-900 pb-32">
      ...
      {view === "home" && (
        <div className="animate-in fade-in duration-500">
          ...
          <div className="max-w-md mx-auto -mt-16 px-6 relative z-30">
            <Glass className="rounded-[48px] p-8 mb-8">
              <div className="flex justify-between items-center mb-10">
                <div>
                  <div className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-1">Status</div>
                  <div className="text-sm font-black text-slate-900">{isWithinTrip ? "Trip en cours 🇻🇳" : "En préparation 📝"}</div>
                </div>
                <button
              ...
              <div className="mb-10">
                <h3 className="text-xl font-black text-slate-900 tracking-tighter mb-4">Équipage</h3>
                <div className="flex items-center justify-between">
                  <FamilyStrip />
                  <div className="text-right">
                    <div className="text-xs font-black text-slate-400 uppercase tracking-tight leading-none mb-1">Les aventuriers</div>
                    <div className="text-lg font-black text-slate-900 leading-none">5 membres</div>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-8 border-t border-slate-50">
                <div className="flex justify-between items-end mb-6">
                  <div>
                    <h3 className="text-xl font-black text-slate-900 tracking-tighter leading-none mb-1">Focus Jour</h3>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Swipe mental</p>
                  </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              ...
              >
                <div className="w-10 h-10 rounded-2xl bg-indigo-500 text-white flex items-center justify-center mb-4"><Lightbulb size={20} /></div>
                <div className="text-sm font-black text-indigo-900">Conseils</div>
                <div className="text-[10px] font-bold text-indigo-400 uppercase">Argent & Culture</div>
              </button>
              <button
                onClick={() => setView("budget")}
                className="p-6 rounded-[40px] bg-amber-50 border border-amber-100 text-left"
              >
                <div className="w-10 h-10 rounded-2xl bg-amber-500 text-white flex items-center justify-center mb-4"><Wallet size={20} /></div>
                <div className="text-sm font-black text-amber-900">Budget</div>
                <div className="text-[10px] font-bold text-amber-400 uppercase">Totaux & Taux</div>
              </button>
            </div>
          </div>
        </div>
      ...
      {/* ITINERARY VIEW */}
      {view === "itinerary" && (
        <div className="animate-in slide-in-from-bottom-8 duration-500 pt-16 px-6">
          <div className="max-w-md mx-auto">
            <div className="flex justify-between items-start mb-12">
              <div>
                <h2 className="text-4xl font-black text-slate-900 tracking-tighter leading-none mb-2">Itinéraire</h2>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em]">Carte par carte</p>
              </div>
            ...
            <div className="mb-10 -mx-6">
              <div className="px-6 mb-4 text-[10px] font-black text-slate-300 uppercase tracking-widest">Filtrer par ville</div>
      ...
      {view === "hotels" && (
        <div className="animate-in slide-in-from-bottom-8 duration-500 pt-16 px-6">
          <div className="max-w-md mx-auto">
            <div className="flex justify-between items-start mb-12">
              <div>
                <h2 className="text-4xl font-black text-slate-900 tracking-tighter leading-none mb-2">Hôtels</h2>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em]">Le repos du guerrier</p>
              </div>
              <button onClick={() => setView("home")} className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-400">
                <X size={20} />
              </button>
      ...
      {view === "guide" && (
        <div className="animate-in slide-in-from-bottom-8 duration-500 pt-16 px-6">
          <div className="max-w-md mx-auto">
            <div className="flex justify-between items-start mb-12">
              <div>
                <h2 className="text-4xl font-black text-slate-900 tracking-tighter leading-none mb-2">Guide</h2>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em]">Food & Logistique</p>
              </div>
              <button onClick={() => setView("home")} className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-400">
                <X size={20} />
              </button>
            ...
            <SimpleListCard title="Manger local" icon={<Utensils size={24} />} items={Object.entries(TRIP_DATA.food).map(([r, f]) => `${r}: ${f.join(", ")}`)} />
            <SimpleListCard title="Vols internes" icon={<Plane size={24} />} items={TRIP_DATA.internal_flights.map(f => `${f.route} • ${f.time} • $${f.group_cost_usd}`)} />
            <AirportGlossary />
          </div>
        </div>
      )}
      ...
      {view === "tips" && (
        <div className="animate-in slide-in-from-bottom-8 duration-500 pt-16 px-6">
          <div className="max-w-md mx-auto">
            <div className="flex justify-between items-start mb-12">
              <div>
                <h2 className="text-4xl font-black text-slate-900 tracking-tighter leading-none mb-2">Conseils</h2>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em]">Argent & Culture</p>
              </div>
              <button onClick={() => setView("home")} className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-400">
                <X size={20} />
              </button>
            ...
            <TipsChecklist />
            <SimpleListCard title="L'argent sur place" icon={<Banknote size={24} />} items={MONEY_TIPS} />
            <SimpleListCard title="Vivre le Vietnam" icon={<Heart size={24} />} items={PRACTICAL_TIPS} />
            <PhrasebookCard />
          </div>
        </div>
      )}
      ...
      {view === "budget" && (
        <div className="animate-in slide-in-from-bottom-8 duration-500 pt-16 px-6">
          <div className="max-w-md mx-auto">
            <div className="flex justify-between items-start mb-12">
              <div>
                <h2 className="text-4xl font-black text-slate-900 tracking-tighter leading-none mb-2">Budget</h2>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em]">Vietnam 2026</p>
              </div>
              <button onClick={() => setView("home")} className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-400">
                <X size={20} />
              </button>
            ...
            <div className="bg-white rounded-[40px] p-8 mb-8 border border-slate-50 shadow-sm">
              <div className="flex justify-between items-end mb-8">
                <div>
                  <h4 className="text-[10px] font-black text-slate-300 uppercase tracking-widest leading-none mb-2">Total Famille</h4>
                  <div className="text-5xl font-black text-slate-900 tracking-tighter leading-none">{formatUSD(budgetSplit.fam.total)}</div>
                </div>
              </div>
              <div className="space-y-4">
                <div className="flex justify-between text-sm font-bold">
                  <span className="text-slate-400 italic">Hôtels</span>
                  <span className="text-slate-900">{formatUSD(budgetSplit.fam.hotels)}</span>
                </div>
                <div className="flex justify-between text-sm font-bold">
                  <span className="text-slate-400 italic">Vols</span>
                  <span className="text-slate-900">{formatUSD(budgetSplit.fam.flights)}</span>
                </div>
                <div className="flex justify-between text-sm font-bold">
                  <span className="text-slate-400 italic">Transferts</span>
                  <span className="text-slate-900">{formatUSD(budgetSplit.fam.transfers)}</span>
                </div>
              </div>
            </div>

            <div className="bg-indigo-600 rounded-[40px] p-8 text-white mb-8 shadow-2xl shadow-indigo-200">
              <h4 className="text-[10px] font-black text-white/40 uppercase tracking-widest leading-none mb-2">Total Claudine</h4>
              <div className="text-5xl font-black tracking-tighter leading-none mb-8">{formatUSD(budgetSplit.claudine.total)}</div>
              <div className="w-12 h-1 bg-white/10 rounded-full" />
            </div>

            <div className="bg-white rounded-[40px] p-8 mb-8 border border-slate-100 shadow-sm">
              <div className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-6 leading-none">Taux de change</div>
              <div className="flex items-center gap-4">
                <input
                  type="number"
                  value={vndPerUsd}
      ...
      <div className="fixed bottom-8 left-6 right-6 z-50">
        <Glass className="rounded-[40px] p-2 flex gap-1">
          {TabsList.map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={`flex-1 flex flex-col items-center justify-center py-4 rounded-3xl transition-all ${
                  active ? "bg-slate-900 text-white scale-105 shadow-xl" : "text-slate-300 hover:text-slate-500"
                }`}
              >
                <Icon size={20} />
              </button>
