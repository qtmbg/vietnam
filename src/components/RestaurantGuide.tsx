import { MapPin, Baby, Leaf, Award, Star } from "lucide-react";
...
// order, a one-line hook, kid/veg badges and a one-tap Maps link. The
// incontournables get a photo (their signature dish) + a "★ Incontournable"
// flag, and any Michelin Guide distinction is shown as its own badge.
          ...
          <div className="mt-3 border-t border-ink-200">
            {g.spots.map((s) => (
              <div key={s.name} className="py-4 border-b border-ink-200">
                {/* Incontournable : photo du plat signature en tête */}
                {s.must && s.img && (
                  <div className="mb-3 overflow-hidden rounded-xl shadow-soft">
                    <SmartImage src={s.img} alt={s.signature} fallback={cover} className="aspect-[16/9] w-full" />
                  </div>
                )}
                  ...
                  <div className="mb-2 flex items-center flex-wrap gap-2">
                    {s.must && (
                      <span className="inline-flex items-center gap-1 rounded-full bg-accent-600 px-2.5 py-0.5 text-[10.5px] font-semibold uppercase tracking-[0.1em] text-white">
                        <Star size={11} fill="currentColor" strokeWidth={0} aria-hidden="true" /> Incontournable
                      </span>
                    )}
                    {s.michelin && (
