...
export const AirportGlossaryCard = ({ items }: { items: AirportGlossaryItem[] }) => (
  <section>
    ...
    <p className="mt-1.5 text-[11px] font-medium uppercase tracking-[0.16em] text-ink-400">Codes + trajets estimés</p>
      ...
      {items.map((a, i) => (
        <div key={i} className="py-4 border-b border-ink-200">
          <div className="flex items-baseline gap-2.5">
            ...
            <span className="font-display italic text-[14px] text-ink-500">{a.city}</span>
          </div>
          <p className="mt-1.5 text-[13px] text-ink-600">{a.airport}</p>
          <div className="mt-2.5 flex items-baseline justify-between gap-4">
            <p className="text-[13px] text-ink-700">
              <span className="text-[11px] uppercase tracking-[0.12em] text-ink-400">Depuis </span>
              {a.fromHotel}
            </p>
            <p className="shrink-0 text-[12px] font-semibold uppercase tracking-[0.12em] text-jade-600 tabular-nums">{a.eta}</p>
          </div>
          {a.note && <p className="mt-1.5 font-display italic text-[13px] text-clay-600">{a.note}</p>}
        </div>
      ))}
    </div>
  </section>
